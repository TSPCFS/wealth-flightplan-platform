...
// "Filled" means a non-empty string or a non-zero number for required fields.
  ...
  for (const section of schema.sections) {
    if (isArraySection(section)) {
      ...
      if (rows.length >= minItems) {
        const requiredCols = section.item_schema?.filter((c) => c.min !== undefined || c.format) ?? section.item_schema ?? [];
        const allComplete = rows.every((row) =>
          requiredCols.every((col) => {
            const v = row?.[col.name];
            return col.type === 'number' ? Number(v ?? 0) > 0 : Boolean(String(v ?? '').trim());
          })
        );
